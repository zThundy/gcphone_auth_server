...
class RoomManager {
    ...
    constructor(client, rooms, mySQLManager, currentServer, roomChannels, config) {
        ...
        rooms.forEach(roomData => {
            ...
            if (currentRoomChannel == undefined) { console.log("Unable to load a room for the user", roomData.user_id + ", since there isn't a room for this user!"); return; }
    ...
    createRoomForID(userId) {
        this.currentServer.channels.create("Gestione Licenza e IP", { "parent": this.config.licenseManagerTicketCategory, "permissionOverwrites": [ { id: this.client.user.id, allow: ['SEND_MESSAGES'] }, { id: userId, allow: ['VIEW_CHANNEL', 'READ_MESSAGE_HISTORY', 'SEND_MESSAGES'] }, { id: this.currentServer.roles.cache.find(r => r.name === '@everyone'), deny: ['VIEW_CHANNEL', 'READ_MESSAGE_HISTORY','SEND_MESSAGES'] } ] }).then(roomChannel => {
